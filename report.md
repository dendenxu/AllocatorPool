...
这一内存资源几乎完全与`MonoMemory`相同，无论是接口还是具体实现。

唯一的区别在于，此内存资源会保有一个尾指针，用以记录已经被返回的内存，因此它支持任意顺序的分配和回收（但必须保证回收的总大小小于等于分配的总大小）。

为了接口的统一，我们会实现与`MonoMemory`相同的接口，但是我们会几乎完全忽略其中的指针参数。（即使有一定的错误检查，这一检查也是极为松散的。）

### 具体实现

我们提供了与`MonoMemory`完全相同的接口。

它们的功能也是基本类似的。

唯一不同的是在调用`free`函数时，用户可以按照不同的顺序来释放相应的内存。没有了`MonoMemory`中的堆栈式限制。
...
这种实现简单又高效，解决了无法分配不定长内存和随机回收分配顺序的问题。但万事都有代价，这种实现极为脆弱。

相信读者在具体实现中会注意到，我们仅仅会在整个内存资源都没有被分配时重新调整`m_head`和`m_tail`的值为零，使得整个内存块变得可用起来
